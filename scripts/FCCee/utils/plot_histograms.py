...
def plot_histo(h, h_ref, match, n_dir, n_histo):
    ...
    # Create a legend
    legend = ROOT.TLegend(
        0.755, 0.675, 0.93, 0.775
    )  # this is the position of the legend (I usualy do trial and error to place it properly)
        ...
        for n_histo, h_name in enumerate(file_struct[dir]):
            ...
            if check_ref:
                if refDir:
                    ...
                    histo_ref = refDir
                if histo_ref:
                    match = comparison_module.compare_histos(
                        histo, histo_ref, args.SL, args.test
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--outputPath",
        type=str,
        help="The name of the directory where to save output files",
        default="./",
    )
    parser.add_argument(
        "-r",
    ...
    parser.add_argument(
        ...
        "--mail",
        type=str,
        help="The name of the file where to store the body of the email",
        default="",
    )
    parser.add_argument(
        "--no_save", action="store_true", help="Do not save output arrays"
    )
    parser.add_argument("--show", action="store_true", help="Plot output histograms")
    parser.add_argument(
        "--test",
        type=str,
        help=f"Test to check compatibility of histograms. Possible options are: chi2, KS, identical",
    )
    parser.add_argument(
        "--SL",
        type=float,
