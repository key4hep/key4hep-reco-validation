...
def Chi2test(h1, h2, SL):
    ...
    if p_val < 1 - SL:
        ...
        match = True
    print(
        f"P-value for histo {h1.GetName()}: {p_val} -->  match test {'passed' if match else 'failed'}"
    )
...
def Identical(h1, h2, SL):
    ...
    for bin_num in range(1, h1.GetNbinsX() + 1):
        ...
        if bin_edge1 != bin_edge2:
            print(
                f"Bin edges do not match at bin {bin_num}: {bin_edge1} vs {bin_edge2}"
            )
            return False

        if bin_content1 != bin_content2:
            print(
                f"Bin contents do not match at bin {bin_num}: {bin_content1} vs {bin_content2}"
            )
            return False
...
tests = {"chi2": Chi2test, "KS": KStest, "identical": Identical}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--inputFile",
        type=str,
        help="The name of the file with new histos to check",
        default="ARC_analysis.root",
    )
    parser.add_argument(
        "-r",
        "--referenceFile",
        type=str,
        help="The name of the file containing reference histos",
        default="",
    )
    parser.add_argument(
        "-h", "--histo", type=str, help="The name of the histogram to check", default=""
    )
    parser.add_argument(
        "--SL",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        choices=tests.keys(),
        help=f"Test to check compatibility of histograms. Possible options are: {dict_keys}",
    )
